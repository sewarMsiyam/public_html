...
class DataGrid 
{
	...
	*/
	public function __construct($_db, $image_path = '')
	{
		$this->_db = $_db;
	...
	public function addStandardControl($type, $action, $action_type = self::TYPE_ONCLICK)
	{
		...
		switch ($type)
		{
			case self::STDCTRL_EDIT:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_edit . '" alt="'.$LANG[edit].'" title="'.$LANG[edit].'" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_DELETE:
				$this->controls[] = ' &nbsp;<a ' . $action . '><img src="' . $this->image_path . $this->img_delete . '" alt="'.$LANG[del].'" title="'.$LANG[del].'" class="tbl-control-image"></a>';
				break;
			default:
	...
	*/
	//by maaking
	private function parseVariables($row, $act)
	{
		$row = array ($row);
		...
		foreach ($headers as $t)
		{
			...
			if ($this->hide_order)
				...
				if ($this->order and $this->order['Column'] == $t)
					$order = ($this->order['Order'] == self::ORDER_ASC) ? self::ORDER_DESC : self::ORDER_ASC;
				else
									...
									)))
			{
				...
				echo '&nbsp;<a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'filter.gif" class="tbl-filter-image" alt="'.$LANG['filter'].'" title="'.$LANG['filter'].'"></a><br><div class="tbl-filter-box" id="filter-' . $t . '" style="display:' . $filter_display . '"><input type="text" size="6" id="filter-value-' . $t . '" value="'.$filter_value.'">&nbsp;<a href="javascript:;" onclick="tblSetFilter(\'' . $t . '\')">'.$LANG['filter'].'</a></div>';
			}
	...
	*/
	...
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		echo "\n\n\n<tfoot>\n\n\n".'<tr class="tbl-footer">'."\n\t".'<td class="tbl-nav" colspan="' . $this->column_count . '"><table width="100%" class="tbl-footer"><tr><td width="33%" class="tbl-found">'.$LANG['found'].' '.$LANG['on'].' <em>' . $this->row_count . '</em> '.$LANG['result'].'/'.$LANG['results'];

		if ($this->row_count > 0)
			echo ', '.$LANG[showing].' <em>' . $first . '</em> '.$LANG[to].' <em>' . $last . '</em>';

		echo '</td>'."\r\t".'<td wdith="33%" class="tbl-pages">';

		// Handle results that span multiple pages
		if ($this->row_count > $this->results_per_page)
		{
			if ($this->page > 1)
				echo '<a href="javascript:;" onclick="tblSetPage(1)"><img src="' . $this->image_path . 'arrow_first.gif" class="tbl-arrows" alt="&lt;&lt;" title="'.$LANG[first_page].'"></a><a href="javascript:;" onclick="tblSetPage(' . ($this->page - 1) . ')"><img src="' . $this->image_path . 'arrow_left.gif" class="tbl-arrows" alt="&lt;" title="'.$LANG[prev_page].'"></a>';
			else
				echo '<img src="' . $this->image_path . 'arrow_first_disabled.gif" class="tbl-arrows" alt="&lt;&lt;" title="'.$LANG[first_page].'"><img src="' . $this->image_path . 'arrow_left_disabled.gif" class="tbl-arrows" alt="&lt;" title="'.$LANG[prev_page].'">';
			...
			if ($this->page < $pages)
				echo '<a href="javascript:;" onclick="tblSetPage(' . ($this->page + 1) . ')"><img src="' . $this->image_path . 'arrow_right.gif" class="tbl-arrows" alt="&gt;" title="'.$LANG[next_page].'"></a><a href="javascript:;" onclick="tblSetPage(' . $pages . ')"><img src="' . $this->image_path . 'arrow_last.gif" class="tbl-arrows" alt="&gt;&gt;" title="'.$LANG[last_page].'"></a>';
			else
				echo '<img src="' . $this->image_path . 'arrow_right_disabled.gif" class="tbl-arrows" alt="&gt;" title="'.$LANG[next_page].'"><img src="' . $this->image_path . 'arrow_last_disabled.gif" class="tbl-arrows" alt="&gt;&gt;" title="'.$LANG[last_page].'">';
		}

		echo '</td>'."\n\t".'<td width="33%" class="tbl-page">';
	...
	*/
	//by maaking
	private function buildControls($row)
	{
			$row = array($row);
	...
	*/
	public function printTable()
	{
		...
		$this->result = $this->_db->sql_query($query . ' ' . $order . ' ' . $limit, false);
		...
		if ($this->row_count == 0)
			...
			while ($row = $this->_db->sql_fetchrow($this->result))
			{
	...
	public static function useAjaxTable($responce = '')
	{
		...
		echo "return xmlHttp; }\n";
		echo "function stateChanged() { if (xmlHttp.readyState == 4) { document.getElementById('datagrid').innerHTML = xmlHttp.responseText; } }\n";
