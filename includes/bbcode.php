...
//====
	...
	$text = str_replace("[fade]", $bbcode_tpl['fade_open'], $text);
	$text = str_replace("[/fade]", $bbcode_tpl['fade_close'], $text);

	// [ram]Ram URL[/ram] code..
	$patterns[] = "#\[ram width=([0-6]?[0-9]?[0-9]) height=([0-4]?[0-9]?[0-9])\](.*?)\[/ram\]#si";
	$replacements[] = $bbcode_tpl['ram'];

	// [stream]Sound URL[/stream] code..
	$patterns[] = "#\[stream\](.*?)\[/stream\]#si";
	...
	$patterns[] = "#\[flash width=([0-6]?[0-9]?[0-9]) height=([0-4]?[0-9]?[0-9])\](.*?)\[/flash\]#si";
	$replacements[] = $bbcode_tpl['flash'];

	// [video width=X height=X]Video URL[/video] code..
	$patterns[] = "#\[video width=([0-6]?[0-9]?[0-9]) height=([0-4]?[0-9]?[0-9])\](.*?)\[/video\]#si";
	...
	$text = preg_replace("/\[marq=(left|right|up|down)\]/si", $bbcode_tpl['marq_open'], $text);
	$text = str_replace("[/marq]", $bbcode_tpl['marq_close'], $text);

	// [table=blah]Table[/table] code..
	$text = preg_replace("/\[table=(.*?)\]/si", $bbcode_tpl['table_open'], $text);
	$text = str_replace("[/table]", $bbcode_tpl['table_close'], $text);

	// [cell=blah]Cell[/table] code..
	$text = preg_replace("/\[cell=(.*?)\]/si", $bbcode_tpl['cell_open'], $text);
	$text = str_replace("[/cell]", $bbcode_tpl['cell_close'], $text);

	// [font=fonttype]text[/font] code..
	$text = preg_replace("/\[font=(.*?)\]/si", $bbcode_tpl['font_open'], $text);
	$text = str_replace("[/font]", $bbcode_tpl['font_close'], $text);

	// [hr]
	$text = str_replace("[hr]", $bbcode_tpl['hr'], $text);
	...
	$text = str_replace("[spoil]", $bbcode_tpl['spoil_open'], $text);
	$text = str_replace("[/spoil]", $bbcode_tpl['spoil_close'], $text);
...
function bbencode_first_pass($text, $uid)
{
	...
	$text = bbencode_first_pass_pda($text, $uid, '/\[quote=(\\\".*?\\\")\]/is', '[/quote]', '', false, '', "[quote=\\1]");
...
//====
	// [fade]Faded Text[/fade] code..
	$text = preg_replace("#\[fade\](.*?)\[/fade\]#si", "[fade]\\1[/fade]", $text);

	// [align=left/center/right/justify]Formatted Code[/align] code..
	$text = preg_replace("#\[align=(left|right|center|justify)\](.*?)\[/align\]#si", "[align=\\1]\\2[/align]", $text);

	 // [marquee=left/right/up/down]Marquee Code[/marquee] code..
	$text = preg_replace("#\[marq=(left|right|up|down)\](.*?)\[/marq\]#si", "[marq=\\1]\\2[/marq]", $text);

	// [table=blah]Table[/table] code..
	$text = preg_replace("#\[table=(.*?)\](.*?)\[/table\]#si", "[table=\\1]\\2[/table]", $text);

	// [cell=blah]Cell[/table] code..
	$text = preg_replace("#\[cell=(.*?)\](.*?)\[/cell\]#si", "[cell=\\1]\\2[/cell]", $text);

	// [font=fonttype]text[/font] code..
	$text = preg_replace("#\[font=(.*?)\](.*?)\[/font\]#si", "[font=\\1]\\2[/font]", $text);

	// [ram]Ram URL[/ram] code..
	$text = preg_replace("#\[ram\](.*?)\[/ram\]#si", "[ram]\\1[/ram]", $text);

	// [stream]Sound URL[/stream] code..
	$text = preg_replace("#\[stream\](.*?)\[/stream\]#si", "[stream]\\1[/stream]", $text);

	// [web]Web Iframe URL[/web] code..
	//$text = preg_replace("#\[web\](http(s)?://)([a-z0-9\-\.,\?!%\*_\#:;~\\&$@\/=\+]+)\[/web\]#si", "[web]\\1\\3[/web]", $text);

	// [flash width=X height=X]Flash URL[/flash] code..
	$text = preg_replace("#\[flash width=([0-6]?[0-9]?[0-9]) height=([0-4]?[0-9]?[0-9])\](([a-z]+?)://([^, \n\r]+))\[\/flash\]#si","[flash width=\\1 height=\\2\]\\3[/flash]", $text);

	// [video width=X height=X]Video URL[/video] code..
	$text = preg_replace("#\[video width=([0-6]?[0-9]?[0-9]) height=([0-4]?[0-9]?[0-9])\](([a-z]+?)://([^, \n\r]+))\[\/video\]#si","[video width=\\1 height=\\2\]\\3[/video]", $text);
	...
	// [spoil]Spoiler[/spoil] code..
	$text = preg_replace("#\[spoil\](.*?)\[/spoil\]#si", "[spoil]\\1[/spoil]", $text);
