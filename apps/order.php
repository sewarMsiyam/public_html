...
class Order_App extends Intro_Apps 
{
	...
	function paynow()
	{
		...
		TableOpen();
			...
			$data['ord_amount'] = (float) $this->total();
		...
		elseif($paymentmethod == 10){
			$gate = $this->getGateway($paymentmethod);
			$discount= $gate['gate_discount'];
			echo $this->BankAccount($gate,$total,$desc,$fname,$orderid);			
		}
				...
				<div class=\"card bg-light m-auto\" style=' max-width:60rem;'>
					  ...
					  <div class=\"card-body\">
						<center><h3>Charg By USDT </h3></center>
							...
							<br>
							<center>
							<a href=\"https://www.buyformula.net/$lang/account/orders\" class='btn btn-warning mx-3 font-weight-bolder'>{$intro->lang['backtoorder']}</a>
							<a href=\"https://www.buyformula.net/$lang/order/success\" class='btn btn-success font-weight-bolder mx-3 '>{$intro->lang['donepay']}</a>
							</center>
						</p>
	...
	function BankAccount($gate,$amound,$desc,$fname,$orderid){
		global $intro,$array;
		$lang = $intro->maa->lang;
		//var_dump($gate);
		...
		echo"<div class=\"container\">
			<div class=\"card bg-light m-auto\" style=' max-width:60rem;'>
				  <div class=\"card-header bg-success \">
					<center><h2 class='text-white' > ".$intro->lang['total_order']." :  ". $intro->maa->Currency_amount($this->total()) ." </h2>	</center> 
				  </div>
				  <div class=\"card-body\">
					<center> ".$gate['gate_bodytext']." </center>
   ...
   <!-- <script src="https://cube.paysky.io/Portal/Transactions/LightboxNew/CallLightBoxByJS/Lightbox.js"></script>   Staging --> 
    ...
    Lightbox.Checkout.configure = {
        ...
        completeCallback: function (data) {
            ...
            console.log(data);
			 window.location.href = "https://www.buyformula.net/<?=$lang?>/order/success";
        },
					...
					$(document).ready(function(){
						...
						$("#reg_form").on("submit", function(e){
							...
							$.ajax({
								...
								data: $(this).serialize(),
								beforeSend: function(){
									$("#returndata").html("<p class='text-info'>Loading ...</p>");
								},
	...
	function doRegister() {
		...
		if($intro->auth->check_email($email) == true ){ 
			echo "<div class='error' style='color:red'>{$intro->lang['existingemail']} </div>";die();
		}
		...
		$data["password"] = $intro->pwd('123456');
		...
		$data2["ord_userid"] = $userid;
		$intro->db->update(PREFIX."_orders",$data2,"ord_id=$orderid");
		...
		if ($intro->auth->check_user_and_login($email, $password)) {
			...
			unset($password);
			echo "<br/><font color=red><center>" . get_msg('login_error') . "</cener></font>";
			die();
		}
